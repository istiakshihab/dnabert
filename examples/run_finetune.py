...
MODEL_CLASSES = {
    ...
    "dnalongcat":
    (BertConfig, BertForLongSequenceClassificationCat, DNATokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    warmup_steps = args.warmup_steps if args.warmup_percent == 0 else int(
        args.warmup_percent * t_total)
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice":
                if args.do_ensemble_pred:
                    ...
                    probs = softmax(torch.tensor(
                        preds, dtype=torch.float32))[:, 1].numpy()
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        args.pred_batch_size = args.per_gpu_pred_batch_size * max(
            1, args.n_gpu)
        # args.pred_batch_size = 8 * max(1, 1)
...
def visualize(args, model, tokenizer, kmer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        if args.task_name != "dnasplice":
            preds = np.zeros([len(pred_dataset), 63])
        else:
            preds = np.zeros([len(pred_dataset), 63])
        attention_scores = np.zeros(
            [len(pred_dataset), 12, args.max_seq_length, args.max_seq_length])

        for index, batch in enumerate(tqdm(pred_dataloader,
                                           desc="Predicting")):
            model.eval()
            batch = tuple(t.to(args.device) for t in batch)
            ...
            with torch.no_grad():
                ...
                # print("logits : ", len(logits[0]))

                preds[index * batch_size:index * batch_size +
                      len(batch[0]), :] = logits.detach().cpu().numpy()
                attention_scores[
                    index * batch_size:index * batch_size +
                    len(batch[0]), :, :, :] = attention.cpu().numpy()
                # if preds is None:
                #     preds = logits.detach().cpu().numpy()
                # else:
...
def main():
    ...
    parser.add_argument("--do_visualize",
                        action="store_true",
                        help="Whether to calculate attention score.")
    parser.add_argument("--visualize_train",
                        action="store_true",
                        help="Whether to visualize train.tsv or dev.tsv.")
    parser.add_argument(
        "--do_ensemble_pred",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        help=
        "set this to a positive integet if you want to perfrom early stop. The model will stop \
    ...
    parser.add_argument("--attention_probs_dropout_prob",
                        default=0.1,
                        type=float,
                        help="Dropout rate of attention.")
    parser.add_argument("--hidden_dropout_prob",
                        default=0.1,
                        type=float,
                        help="Dropout rate of intermidiete layer.")
    ...
    parser.add_argument("--rnn",
                        default="lstm",
                        type=str,
                        help="What kind of RNN to use")
    parser.add_argument("--num_rnn_layer",
                        default=2,
                        type=int,
                        help="Number of rnn layers in dnalong model.")
    parser.add_argument("--rnn_hidden",
                        default=768,
                        type=int,
                        help="Number of hidden unit in a rnn layer.")
    parser.add_argument(
        "--num_train_epochs",
        default=3.0,
        type=float,
    ...
    parser.add_argument("--warmup_percent",
                        default=0,
                        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The model used to do visualization. If None, use 3456.",
    )
...
# Setup CUDA, GPU & distributed training
    ...
    if args.do_visualize and args.local_rank in [-1, 0]:
        visualization_models = [3, 4, 5, 6] if not args.visualize_models else [
            args.visualize_models
        ]
        ...
        for kmer in visualization_models:
            ...
            logger.info(
                "Calculate attention score using the following checkpoint: %s",
                checkpoint)
            prefix = checkpoint.split(
                "/")[-1] if checkpoint.find("checkpoint") != -1 else ""
            config = config_class.from_pretrained(
            ...
            model.to(args.device)
            attention_scores, probs = visualize(args,
                                                model,
                                                tokenizer,
                                                prefix=prefix,
                                                kmer=kmer)
    ...
    if args.do_ensemble_pred and args.local_rank in [-1, 0]:
        ...
        for kmer in range(3, 7):
            ...
            if kmer == 3:
                ...
                args.data_dir = args.data_dir.replace("/" + str(kmer - 1),
                                                      "/" + str(kmer))
            ...
            if args.result_dir.split('/')[-1] == "test.npy":
                ...
                results, eval_task, _, out_label_ids, probs = evaluate(
                    args, model, tokenizer, prefix=prefix, evaluate=False)
            else:
                raise ValueError(
                    "file name in result_dir should be either test.npy or train.npy"
                )
        ...
        ensemble_results = compute_metrics(eval_task, all_preds, out_label_ids,
                                           all_probs[:, 1])
